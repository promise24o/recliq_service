...
@Injectable()
export class GetPendingVehiclesUseCase {
  constructor(
  ...
  async execute(query: PendingVehiclesQueryDto): Promise<{
    vehicles: (VehicleApprovalResponseDto & { user?: { id: string; name: string; email: string; phone: string; role: string } })[];
    pagination: {
      page: number;
      limit: number;
      total: number;
    ...
    const { page = 1, limit = 20, status = VehicleStatus.PENDING } = query;
